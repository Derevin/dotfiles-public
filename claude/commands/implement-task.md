...
6. **Converge.** Invoke `/forkwc-converge`, handing it the implement fork's return verbatim plus any steer the user has given since. That handoff is everything it knows about what's already settled; without it the loop spends its rounds reopening decisions the implementer already made and recorded. Forked, it loops one generalist reviewer against the diff and adopts or drops to a fixed point. The diff, the rounds, and the edit churn stay in the fork; only the net comes back.
