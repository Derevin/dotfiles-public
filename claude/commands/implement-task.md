...
7. **Converge.** Invoke `/converge-review`: one generalist reads the whole diff and proposes, you adopt or drop, looped to a fixed point. Keep the diff out of your own context — dispatching is what makes it cheap.

8. **Deep review.** Once, as a backstop, invoke `/review-branch-medium` — the multi-perspective fan-out that the loop's single generalist can't give you. Sort every finding through the gate below: apply the **Apply** bucket in place and uncommitted, resolve the **Drop** bucket yourself, carry only the **Ask** bucket to the handoff.
