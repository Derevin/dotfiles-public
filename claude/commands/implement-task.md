...
5. **Implement.** Invoke `/fork-implement`, seeding it with whatever this conversation settled that the task file doesn't record — a premise the user corrected, a scope cut, a list of sites agreed in chat. Pass the settled subset, not a transcript: the fork starts cold on purpose, and the seed is the only part of this conversation it gets, so anything floated and dropped belongs nowhere near it. Empty is right when the file is current. Forked, it takes that seed plus the task file and the context store as its brief, implements the plan, proves the tests red-green, and returns the decisions the diff can't show. The reading and build output stay behind in the fork, so the review and handoff below run on a context that carries no implementation baggage. If it returns a question instead of an implementation, the brief had a gap — take it to the user, then dispatch again with their answer in the seed; re-dispatching against an unchanged brief earns the same question.

6. **Converge.** Invoke `/forkwc-converge`: it dispatches a fork that inherits this conversation — the plan, the steers, and what the implementer decided — and loops one generalist reviewer against the diff, adopting or dropping to a fixed point. The diff, the rounds, and the edit churn stay in the fork; only the net comes back.
