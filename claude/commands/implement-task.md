...
6. **Converge.** Invoke `/fork-converge`: forked, it loops one generalist reviewer against the diff and adopts or drops to a fixed point. The diff, the rounds, and the edit churn stay in the fork; only the net comes back.

7. **Deep review.** Once, as a backstop, invoke `/fork-review` — the multi-perspective fan-out that the loop's single generalist can't give you, run in its own fork with the Apply/Drop/Ask gate applied inside it. It returns only the judgment calls that survived, so carry those to the handoff. It comes in cold by design: what converge dropped died with that fork, and that's what makes this an independent check rather than a second lap.
