...
2. **Read.** Read the task file at `~/repos/tasks/<project>/active/<filename>` so you know its existing structure (sections, prior status notes, user-written Context).
...
4. **Update.** Edit the task file in place:
   ...
   - **Refresh or add**: a one-line `## Status: <state>` and the progress sections that match the task's existing shape (e.g. `## What's done` / `## What's left` / `## How to pick up <X>`). Don't invent boilerplate sections the task doesn't need.
