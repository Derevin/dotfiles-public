...
The user can invoke this whenever. Self-invoke only as a step of the `/plan-task` flow, where `/fork-snapshot` calls it — not on your own initiative mid-implementation; commits and the conversation already carry that state.
