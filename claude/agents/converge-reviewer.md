...
Run `review-diff.sh` (no arguments, no wrappers) for the commit log and full diff. Read the task file and context store you were given — they carry the intent the diff is meant to serve. Read into the surrounding code wherever the diff alone can't tell you whether something is a problem.
