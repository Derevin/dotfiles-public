...
#
# Usage: lab-split.sh <h|v> [extra split-window args...]
...
# lab — landing in its checkout with claude-less bash. Otherwise it behaves like
# the original binding: a host shell that inherits pane_current_path.
set -euo pipefail
if [[ "${1:-}" == "--help" ]]; then
    echo "Lab-aware split for tmux bindings; the new pane follows the caller's lab."
    echo "Usage: lab-split.sh <h|v> [extra split-window args...]"
    exit 0
fi
...
lab=$(tmux show-options -pv @lab 2>/dev/null || true)
...
if [ -n "$lab" ]; then
    ...
    # resolution.
    exec tmux split-window "$DIR" $BEFORE -c "$cwd" "$@" "lab-shell $lab"
else
