#!/usr/bin/env bash
# Save the active pane's editor (micro/nvim) before an Alt+w close, so unsaved
...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    echo "Save the active pane's editor (micro/nvim) before it is closed."
...
case "$(tmux display-message -p '#{pane_current_command}')" in
    micro)
        ...
        ;;
    nvim|vim)
        # :wa writes every modified buffer, not just the active one.
        tmux send-keys Escape ':wa' Enter
        ;;
    *)
        exit 0
        ;;
