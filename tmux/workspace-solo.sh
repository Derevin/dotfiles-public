...
if [[ $TAKEOVER -eq 1 ]]; then
    ...
    tmux send-keys -t "$W.1" "cd $DIR && CLAUDE_LABEL=$BASE claude --effort max${CLAUDE_ARG:+ '$CLAUDE_ARG'}" Enter
else
    tmux send-keys -t "$W.1" "CLAUDE_LABEL=$BASE claude --effort max${CLAUDE_ARG:+ '$CLAUDE_ARG'}" Enter
fi
