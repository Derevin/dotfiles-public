...
declare -F lab_resolve >/dev/null || { echo "workspace-solo.sh: cannot find lab-lib.sh" >&2; exit 1; }
...
if [[ $TAKEOVER -eq 1 ]]; then
    ...
    tmux send-keys -t "$W.1" "cd $DIR && $(lab_claude_cmd "$BASE" "$CLAUDE_ARG")" Enter
else
    tmux send-keys -t "$W.1" "$(lab_claude_cmd "$BASE" "$CLAUDE_ARG")" Enter
fi
