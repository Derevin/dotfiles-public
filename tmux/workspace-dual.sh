...
if [[ $TAKEOVER -eq 1 ]]; then
    ...
    tmux send-keys -t "$W.1" "cd $DIR && claude --effort max -n ${BASE}1" Enter
else
    tmux send-keys -t "$W.1" "claude --effort max -n ${BASE}1" Enter
fi

tmux send-keys -t "$W.2" "claude --effort max -n ${BASE}2" Enter
